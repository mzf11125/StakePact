...
pub mod agentbond {
    ...
    pub fn create_task(
        ...
        let clock = Clock::get()?;
        require!(deadline > clock.unix_timestamp, AgentBondError::DeadlinePassed);
    ...
    pub fn accept_task(ctx: Context<AcceptTask>, _task_id: u64) -> Result<()> {
        ...
        require!(task.status == TaskStatus::Open, AgentBondError::InvalidStatus);
        require!(clock.unix_timestamp < task.deadline, AgentBondError::DeadlinePassed);
    ...
    pub fn submit_result(
        ...
        let task = &mut ctx.accounts.task;
        require!(task.status == TaskStatus::Accepted, AgentBondError::InvalidStatus);
    ...
    pub fn verify_result(
        ...
        require!(quality_score <= 100, AgentBondError::InvalidScore);

        let task = &mut ctx.accounts.task;
        require!(task.status == TaskStatus::Submitted, AgentBondError::InvalidStatus);
    ...
    pub fn claim_expired(ctx: Context<ClaimExpired>, _task_id: u64) -> Result<()> {
        ...
        require!(task.status == TaskStatus::Accepted, AgentBondError::InvalidStatus);
        require!(clock.unix_timestamp > task.deadline, AgentBondError::DeadlineNotPassed);
